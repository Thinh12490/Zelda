...
function Dungeon:beginShifting(shiftX, shiftY)
    ...
    for k, carried in ipairs(self.player.carriedItems) do   
        carriedTweens[carried.item] = {
            x = playerX,
            y = playerY - carried.item.height + 10
        }
    end
    ...
    local allTweens = self:tableMerge({
        [self] = {cameraX = shiftX, cameraY = shiftY},
        [self.player] = {x = playerX, y = playerY}
    }, carriedTweens)
    
    Timer.tween(1, allTweens):finish(function()
...
function Dungeon:tableMerge(t1, t2)
    local result = {}
...
function Dungeon:resetState()
    if #self.player.carriedItems > 0 then
